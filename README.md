...
| 심민재 | 조원 | 결제 페이지, 장바구니                                      |장바구니: 장바구니 화면 UI 구성 및 사용자가 선택한 상품에 대한 수량 증가/감소, 삭제, 전체 선택/해제 기능 구현.<br>실시간 총 결제 금액 계산 로직 개발 및 상태 관리 구현(localStorage와의 연동 포함).<br>비회원 장바구니 기능 구현 후 로그인 시 기존 장바구니와의 병합 처리 구현.<br>결제 페이지: 선택된 장바구니 상품 정보를 결제 페이지로 전달하고, 사용자 입력 정보(배송지, 수취인 정보 등)에 대한 유효성 검증 로직 구성.<br>결제 요청 API 연동 및 응답 처리 흐름 구현. 결제 성공 시 주문 등록 및 장바구니 초기화 기능까지 전체 흐름 구현.|
